...
import { MediaReviewProps, ReviewItemProps } from "../../types/mediaReview";
...
const MediaReview = ({ reviews, media, mediaType }: MediaReviewProps) => {
  const { user } = useAppSelector((state) => state.user);
  ...
  const onAddReview = async () => {
    ...
    if (response) {
      ...
      setFilteredReviews([...filteredReviews, response.data]);
  ...
  const onRemoved = (id: number) => {
    if (listReviews.findIndex((e: any) => e.id === id) !== -1) {
      const newListReviews = [...listReviews].filter((e: any) => e.id !== id);
      setListReviews(newListReviews);
      setFilteredReviews([...newListReviews].splice(0, page * skip));
    } else {
      setFilteredReviews([...filteredReviews].filter((e: any) => e.id !== id));
    }
